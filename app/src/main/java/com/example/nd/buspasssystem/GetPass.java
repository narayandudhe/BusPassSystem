...
public class GetPass extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        up=(Button)findViewById(R.id.approvebtn);
        mobile=findViewById(R.id.mobileNumber);
        firebaseAuth=FirebaseAuth.getInstance();
        firebaseDatabase=FirebaseDatabase.getInstance();
        firebaseStorage=FirebaseStorage.getInstance();
        storageReference = firebaseStorage.getReference();
        storageReference.child("profile").child(ssessionId).getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {

            @Override
            public void onSuccess(Uri uri) {
                Toast.makeText(GetPass.this,"image loaded",Toast.LENGTH_SHORT).show();
                Picasso.get().load(uri).into(wimages);
            }
        });
        DatabaseReference databaseReference=firebaseDatabase.getReference("student").child(ssessionId);
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Toast.makeText(GetPass.this,"could not connect",Toast.LENGTH_SHORT).show();
            }
        });
        up.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getBaseContext(), MakePass.class);
                intent.putExtra("EXTRA_SESSION_ID",ssessionId);
